...
export default function Formula3() {
  ...
  const fixedExpenses = transactions?.filter((t: any) => t.category === 'Fixed' && t.type === 'expense')
    .reduce((sum: number, t: any) => sum + t.amount, 0) || 0;

  const variableExpenses = transactions?.filter((t: any) => t.category === 'Variable' && t.type === 'expense')
    .reduce((sum: number, t: any) => sum + t.amount, 0) || 0;

  const investments = transactions?.filter((t: any) => t.category === 'Investment' && t.type === 'expense')
    .reduce((sum: number, t: any) => sum + t.amount, 0) || 0;
      ...
      <div className="space-y-6">
        ...
        <div>
          ...
          <div className="h-2.5 bg-gray-100 rounded-full overflow-hidden">
            <div 
              className={`h-full ${getProgressBarColor('fixed', formula3Data.fixed.percentage)} transition-all duration-300`}
              style={{ width: `${Math.min(formula3Data.fixed.percentage, 100)}%` }}
            ></div>
          </div>
          <div className="flex justify-between text-xs mt-1">
            ...
            <div className="flex">
              <span className={`font-medium ${getTextColor('fixed', formula3Data.fixed.percentage)}`}>
        ...
        <div>
          ...
          <div className="h-2.5 bg-gray-100 rounded-full overflow-hidden">
            <div 
              className={`h-full ${getProgressBarColor('variable', formula3Data.variable.percentage)} transition-all duration-300`}
              style={{ width: `${Math.min(formula3Data.variable.percentage, 100)}%` }}
            ></div>
          </div>
          <div className="flex justify-between text-xs mt-1">
            <div>
            ...
            <div className="flex">
              <span className={`font-medium ${getTextColor('variable', formula3Data.variable.percentage)}`}>
        ...
        <div>
          ...
          <div className="h-2.5 bg-gray-100 rounded-full overflow-hidden">
            <div 
              className={`h-full ${getProgressBarColor('investments', formula3Data.investments.percentage)} transition-all duration-300`}
              style={{ width: `${Math.min(formula3Data.investments.percentage, 100)}%` }}
            ></div>
          </div>
          <div className="flex justify-between text-xs mt-1">
            <div>
            ...
            <div className="flex">
              <span className={`font-medium ${getTextColor('investments', formula3Data.investments.percentage)}`}>
